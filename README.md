...
A reproducible and interactive [RStudio environment](https://rstudio.cloud/project/285535) has been generated for this project, alongside the _cleaned_ data, enabling scripts `1b` and beyond to be run. Packages have been installed and paths slightly modified such that simply opening and sourcing scripts works out of the box. For non-interactive examination of output, several crude PDF reports have been generated and placed in the `Reports` directory. The `Reports` directory also contains a `DataExplorer` EDA report generated using a small sample of the `beneficiary_claims` data (generating a report on the whole dataset would've taken several hours due to its size and so the EDA report is simply a taste of what that might look like).
